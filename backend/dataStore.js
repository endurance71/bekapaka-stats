...
/**
 * Usuwa wyłącznie dane z importu protokołów (model Game i powiązane ślady).
 * Nie dotyka KalkMatch, ligi KALK, kadry ani kont użytkowników.
 */
export async function purgeProtocolData() {
  await ensureSeeded();

  const gamesDeleted = await prisma.game.deleteMany();

  const protocolUrlCleared = await prisma.leagueMatch.updateMany({
    where: { protocolUrl: { not: null } },
    data: { protocolUrl: null }
  });

  const orphanDetailsCleared = await prisma.$executeRaw`
    UPDATE "LeagueMatch"
    SET details = NULL
    WHERE "kalkMatchId" IS NULL
  `;

  const rosterAiReset = await prisma.rosterPlayer.updateMany({
    data: {
      aiDevelopmentSummary: null,
      aiDevelopmentAt: null,
      aiDevelopmentModel: null,
      aiDevelopmentHash: null
    }
  });

  const scoutingReportsDeleted = await prisma.scoutingAiReport.deleteMany();
  const teamBriefingsDeleted = await prisma.teamBriefing.deleteMany();
  ...
  return {
    ...
    protocolUrlCleared: protocolUrlCleared.count,
    orphanLeagueDetailsCleared: Number(orphanDetailsCleared) || 0,
    rosterAiPlansCleared: rosterAiReset.count,
    scoutingReportsDeleted: scoutingReportsDeleted.count,
    teamBriefingsDeleted: teamBriefingsDeleted.count,
    remainingGames,
    kalkMatches: await prisma.kalkMatch.count(),
    kalkPlayerGameLogs: await prisma.kalkPlayerGameLog.count()
  };
}
